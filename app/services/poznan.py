"""
Poznan (Wierzba 2000) discriminant model — bankruptcy predictor.

ZP = 3.26261*X1 + 2.16352*X2 + 0.30141*X3 + 0.68516*X4 - 2.40435

Where:
  X1 = (Current assets - Short-term liabilities) / Total assets (working capital ratio)
  X2 = Net profit / Total assets (ROA)
  X3 = (Net profit + Depreciation) / Total liabilities
  X4 = Current assets / Short-term liabilities (current ratio)
...
MODEL_ID = "poznan_2000_v1"
MODEL_NAME = "poznan"
MODEL_VERSION = "2000_v1"
FEATURE_SET_ID = "poznan_4"
...
def ensure_model_registered() -> None:
    """Insert poznan_2000_v1 into model_registry if not already present."""
    prediction_db.register_model(
        ...
        description=(
            "Poznan (Wierzba 2000) 4-variable "
...
def _score_from_feature_rows(report_id: str, features: list[dict]) -> Optional[dict]:
    ...
    if missing:
        logger.warning(
            "poznan_missing_features",
            extra={
                "event": "poznan_missing_features",
                "report_id": report_id,
                "missing": missing,
            },
        )
...
def score_report(report_id: str) -> Optional[dict]:
    """Score a single report using the Poznan discriminant function.
...
def score_batch(report_ids: Optional[list[str]] = None) -> dict:
    ...
    If report_ids is None, finds reports with computed Poznan features
    ...
    if not report_ids:
        logger.info(
            "poznan_batch_noop",
            extra={"event": "poznan_batch_noop", "model_id": MODEL_ID},
        )
        return {
            "run_id": None,
            "scored": 0,
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "poznan_finalize_failed",
                extra={
                    "event": "poznan_finalize_failed",
                    "model_id": MODEL_ID,
                    "run_id": run_id,
                },
                exc_info=True,
    ...
    logger.info(
        "poznan_batch_completed",
        extra={
            "event": "poznan_batch_completed",
            "run_id": run_id,
            "scored": scored,
            "skipped": skipped,
...
def _find_unscored_reports() -> list[str]:
    """Find reports that have all 4 Poznan features computed but no prediction yet."""
    ...
    rows = conn.execute("""
        ...
        WHERE cf.feature_definition_id IN ('x1_poznan', 'x2_poznan',
                                           'x3_poznan', 'x4_poznan')
