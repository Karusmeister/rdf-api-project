"""
Prusak P1 (2005) discriminant model — bankruptcy predictor.

P1 = 6.5245*X1 + 0.1480*X2 + 0.4061*X3 + 2.1754*X4 - 1.5685

Where:
  X1 = Net profit / Total assets
  X2 = Operating costs / Short-term liabilities
  X3 = Current assets / Total liabilities
  X4 = Operating profit / Revenue
...
MODEL_ID = "prusak_p1_v1"
MODEL_NAME = "prusak_p1"
MODEL_VERSION = "p1_v1"
FEATURE_SET_ID = "prusak_p1_4"
...
def ensure_model_registered() -> None:
    """Insert prusak_p1_v1 into model_registry if not already present."""
    prediction_db.register_model(
        ...
        description=(
            "Prusak P1 (2005) 4-variable discriminant model "
            "for Polish companies"
        ),
        hyperparameters={
            "coefficients": COEFFICIENTS,
            "intercept": INTERCEPT,
            "cutoffs": {"critical": -0.13, "medium": 0.65},
        },
        is_baseline=False,
    )
...
def classify(z_score: float) -> tuple[int, str]:
    """Map P1-score to (classification, risk_category).

    Per the published model:
      P1 < -0.13  → bankruptcy zone (critical)
      -0.13 <= P1 < 0.65  → grey zone (medium)
      P1 >= 0.65  → safe zone (low)
    """
    if z_score < -0.13:
        return (1, "critical")
    elif z_score < 0.65:
...
def _score_from_feature_rows(report_id: str, features: list[dict]) -> Optional[dict]:
    ...
    if missing:
        logger.warning(
            "prusak_missing_features",
            extra={
                "event": "prusak_missing_features",
                "report_id": report_id,
                "missing": missing,
            },
        )
...
def score_report(report_id: str) -> Optional[dict]:
    """Score a single report using the Prusak P1 discriminant function.
...
def score_batch(report_ids: Optional[list[str]] = None) -> dict:
    ...
    If report_ids is None, finds reports with computed Prusak P1 features
    ...
    if not report_ids:
        logger.info(
            "prusak_batch_noop",
            extra={"event": "prusak_batch_noop", "model_id": MODEL_ID},
        )
        return {
            "run_id": None,
            "scored": 0,
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "prusak_finalize_failed",
                extra={
                    "event": "prusak_finalize_failed",
                    "model_id": MODEL_ID,
                    "run_id": run_id,
                },
                exc_info=True,
    ...
    logger.info(
        "prusak_batch_completed",
        extra={
            "event": "prusak_batch_completed",
            "run_id": run_id,
            "scored": scored,
            "skipped": skipped,
...
def _find_unscored_reports() -> list[str]:
    """Find reports that have all 4 Prusak P1 features computed but no prediction yet."""
    ...
    rows = conn.execute("""
        ...
        WHERE cf.feature_definition_id IN ('x1_prusak', 'x2_prusak',
                                           'x3_prusak', 'x4_prusak')
